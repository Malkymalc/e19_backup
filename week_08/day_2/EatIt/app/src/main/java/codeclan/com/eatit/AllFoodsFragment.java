...
public class AllFoodsFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.activity_all_foods_fragment, container, false);

        // 1. Get the data we need to populate the list view
        DBHandler db = DBHandler.getInstance(getContext());
        ArrayList<Food> allFoods = db.getAllfoods();

        // 2. Instantiate an instance of the "AllFoodsListAdapter" ListAdapter class
        AllFoodsListAdapter allFoodsListAdapter = new AllFoodsListAdapter(getContext(), allFoods);

        // 3. Get the listView component and set it's adapter to the allFoodsListAdapter instance in (2.)
        ListView listView = view.findViewById(R.id.all_foods_list);
        listView.setAdapter(allFoodsListAdapter);
